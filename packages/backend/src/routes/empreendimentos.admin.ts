...
const empreendimentoSchema = z.object({
  ...
  precoMin: num,
  precoMax: num,
  percentualVendido: z.coerce.number().min(0).max(100).optional().or(z.literal('')),
...
function toDbData(data: z.infer<typeof empreendimentoSchema>) {
  return {
    ...
    percentual_vendido: toInt(data.percentualVendido),
    data_inicio: data.dataInicio ? new Date(data.dataInicio) : null,
